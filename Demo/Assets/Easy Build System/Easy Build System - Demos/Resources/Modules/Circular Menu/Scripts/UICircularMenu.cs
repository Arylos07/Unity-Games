...
public class UICircularMenu : MonoBehaviour
{
    ...
    private CustomBlurOptimized Blur;
    ...
    private void Awake()
    {
        Blur = FindObjectOfType<CustomBlurOptimized>();
    ...
    {
        BaseBuilder.Instance.ChangeMode(BuildMode.Placement);

        BaseBuilder.Instance.SelectedIndex = BuildManager.Instance.BuildCollection.Parts.FindIndex(entry => entry.Name == name);

        BaseBuilder.Instance.SelectPrefab(BuildManager.Instance.GetPart(name));
    ...
    {
        BaseBuilder.Instance.ChangeMode(BuildMode.Destruction);
    }
